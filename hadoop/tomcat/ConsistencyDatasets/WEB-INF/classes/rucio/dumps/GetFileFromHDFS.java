...
public class GetFileFromHDFS extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (rse_param == null) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "You have to specify an RSE.");
            return;
        }

        String date_param = request.getParameter("date");

        System.out.println(rse_param);
        String date = null;
        String path = null;
        if (date_param != null) {
            try {
                ...
                response.sendError(HttpServletResponse.SC_BAD_REQUEST,
                        "Your date parameter is malformed. The expected format is \"DD-MM-YYYY\"");
                return;
            }
        } else {
        ...
        if (rse_param.startsWith("*")) {
            response.setContentType("application/bz2");
            response.setHeader("Content-Disposition", "attachment;filename=" + "consistency_datasets_" + date + ".bz2");
        } else {
            response.setContentType("application/text");
            response.setHeader("Content-Disposition", "attachment;filename=" + rse_param + "_" + date);
        }
        ...
        /*
         * FSDataInputStream in = fs.open(status[0].getPath());
         * 
         * byte b[] = new byte[4096]; int len;
         * 
         * while ((len = in.read(b)) > 0) { if (len < 4096) {
         * output.write(Arrays.copyOfRange(b, 0, len)); } else { output.write(b); } }
         * in.close();
         */
    }
    ...
    private String generatePath(String rse, String date) {
        ...
        path.append(date);
        path.append("/consistency_datasets");
        System.out.println(rse);
        if (rse.startsWith("*")) {
            path.append("_full.bz2/");
        } else {
            path.append("/");
            path.append(rse);
            path.append("/");
